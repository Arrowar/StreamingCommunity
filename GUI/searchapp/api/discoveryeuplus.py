...
from StreamingCommunity.services.discoveryeuplus.scrapper import GetSerieInfo


class DiscoveryEuPlusAPI(BaseStreamingAPI):
    def __init__(self):
        super().__init__()
        self.site_name = "discoveryeuplus"
        self._load_config()
        self._search_fn = None
    
    def _load_config(self):
        """Load site configuration."""
        self.base_url = "https://eu1-prod.disco-api.com"
    ...
    def get_series_metadata(self, media_item: MediaItem) -> Optional[List[Season]]:
        ...
        for season_num in [s.number for s in scrape_serie.seasons_manager.seasons]:
            ...
            print(f"[DiscoveryPlus] Season {season_num}: {len(episodes)} episodes")
